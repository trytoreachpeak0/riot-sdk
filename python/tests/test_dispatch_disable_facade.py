...
_DISABLE_SUCCESS_BODY = '{"code":"0","message":"成功","msgDetail":"","tid":""}'


@pytest.mark.asyncio
async def test_dispatch_disable_succeeds_without_throwing() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200,
            text=_DISABLE_SUCCESS_BODY,
    ...
    async with httpx.AsyncClient(
        ...
        async with RiotSession(options, client=client) as session:
            await session.tasks.dispatch_disable(_DEVICE_KEY)
