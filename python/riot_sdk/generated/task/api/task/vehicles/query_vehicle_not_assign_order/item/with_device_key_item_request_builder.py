...
class WithDeviceKeyItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/task/vehicles/queryVehicleNotAssignOrder/{deviceKey}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/task/vehicles/queryVehicleNotAssignOrder/{deviceKey}", path_parameters)
    
    def by_order_key(self,order_key: str) -> WithOrderKeyItemRequestBuilder:
        """
        Gets an item from the riot_sdk.generated.task.api.task.vehicles.queryVehicleNotAssignOrder.item.item collection
