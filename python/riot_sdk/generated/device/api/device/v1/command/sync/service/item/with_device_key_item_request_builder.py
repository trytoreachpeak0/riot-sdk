...
class WithDeviceKeyItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/device/v1/command/sync/service/{deviceKey}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/device/v1/command/sync/service/{deviceKey}", path_parameters)
    
    def by_service_id(self,service_id: str) -> WithServiceItemRequestBuilder:
        """
        Gets an item from the riot_sdk.generated.device.api.device.v1.command.sync.service.item.item collection
